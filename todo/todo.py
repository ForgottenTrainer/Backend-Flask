...
def get_todo(id):
    db,c = get_db()
    c.execute(
        'select t.id, t.description, t.created_by, t.created_at, u.username from todo t join user u on t.created_by = u.id where t.id = %s',
        (id,)
    )
    todo = c.fetchone()
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if not description:
            error = "Descricion no debe estar vacia"
